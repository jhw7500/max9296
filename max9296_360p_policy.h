 ...
 * 14.80 us for 720p.  That figure is not wrong, but it is not the current
 * default either; which condition produced it is unconfirmed.  Forcing
 * PREVIEW_LINE_TIME (R0x201C) to 14.8 us does reproduce LINE_LENGTH_PCK 1332
 * exactly, so it is a sufficient cause, not a proven one; a preview-resolution
 * difference is ruled out (an 11-point sweep contains no 1332).  The driver
 * never writes R0x201C and
