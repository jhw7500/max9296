...
이 하나가 진입점이고 18개 테스트 파일을 모두 돌린다. **보드가 필요 없다.** 대신
...
| `max9296_cold_init_epoch_test.py` | 실제 prepare·레지스터 표 실행 | 세션 override 폐기·초기화 실패 뒤 같은 전원 주기의 콜드 초기화 차단, 전원 리셋 뒤 재시도 |
