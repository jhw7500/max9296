...
/*
 * SPIKE shim: 5.10.35 does not EXPORT the standard compound type ops
 * (v4l2_ctrl_type_op_validate/_log/_equal) — those were exposed in mainline
 * 5.16 together with the dynamic-array control framework. The std logic still
 * exists internally (std_validate/std_log/std_equal, static in v4l2-ctrls.c).
 * Provide minimal local equivalents for this U32 vertex-map array so the
 * out-of-tree module compiles WITHOUT patching core kernel headers/sources.
 * A production backport would instead backport the 5.16 v4l2-ctrls work.
 */
static bool dw100_spike_ctrl_equal(const struct v4l2_ctrl *ctrl, u32 idx,
				   union v4l2_ctrl_ptr p1,
				   union v4l2_ctrl_ptr p2)
{
	return p1.p_u32[idx] == p2.p_u32[idx];
}

static int dw100_spike_ctrl_validate(const struct v4l2_ctrl *ctrl, u32 idx,
				     union v4l2_ctrl_ptr ptr)
{
	return 0;
}

static void dw100_spike_ctrl_log(const struct v4l2_ctrl *ctrl)
{
}

static const struct v4l2_ctrl_type_ops dw100_ctrl_type_ops = {
	.init = dw100_ctrl_dewarping_map_init,
	.validate = dw100_spike_ctrl_validate,
	.log = dw100_spike_ctrl_log,
	.equal = dw100_spike_ctrl_equal,
};
...
static int dw100_s_fmt(struct dw100_ctx *ctx, struct v4l2_format *f)
{
	...
	if (f->type == V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE) {
		...
		/*
		 * SPIKE shim: v4l2_ctrl_modify_dimensions() (dynamic arrays,
		 * mainline 5.16) is absent in 5.10.35. The resize is skipped, so
		 * the vertex-map control keeps its creation-time dims. This is a
		 * FUNCTIONAL limitation for non-default resolutions — a production
		 * backport must restore a real resize (backport the 5.16
		 * v4l2-ctrls dynamic-array framework, or destroy+recreate the
		 * control on s_fmt). See the dw100-spike report.
		 */
		(void)ctrl;
		(void)dims;
	}
