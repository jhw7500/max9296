...
## 미릴리스 확인 사항 (2026-09-21) — 동작 변경 없음

**2.9(2026-08-28)부터 720p 화각이 넓어져 있다.** 2.9 이전에는 드라이버가 720p 모드
(`2560x720`, `1280x720`)에 `0x1010 = 0x0140`(1.25배 디지털 줌)을 하드코딩으로 썼고,
1080p 모드에는 `0x0100`(1.00배)을 썼다. 최초 벤더 소스(2026-02-03)부터 있던 코드이며
주석이 없어 의도는 알 수 없다. 2.9 가 이 하드코딩을 제거하고 `crop_enable`(기본 false)
뒤로 게이팅하면서 센서 판독 window 가 1536x864 에서 1920x1080 으로 돌아왔다.

- 2.9 는 `crop_enable=false` 에서 `0x1010`/`0x1012`/`0x118c`/`0x118e` 를 **건드리지 않는
  것**을 원칙으로 삼았다. 이 항목 자체(화각 변경 기록)는 동작을 바꾸지 않는다.
  **다만 같은 미릴리스 창의 별도 항목이 그 원칙을 바꿨다** — 아래 "`crop_enable=false` 가
  줌 레지스터를 기본값으로 되쓴다" 를 함께 읽을 것. 이제 false 에서도 해상도별 기본값
  (현재 전 모드 1.00배·중앙)을 기록하므로, 되돌릴 주체가 없어 이전 줌이 남던 문제가 없다.
- 같은 카메라·같은 720p 해상도라도 **2.9 이전 빌드와 이후 빌드는 담기는 범위가 다르다.**
  현장 비교나 이슈 분석에서 이 점을 먼저 확인할 것.
- 부수 효과: 720p 라인타임 14.80 → **11.867 us**, 노출 하한 29.6 → **23.7 us**.
  줌을 다시 켜면(`crop_enable=1, dz=125`) 하한도 29.6 us 로 함께 올라간다.
- **2.12 의 "720p 라인타임 14.80 us" 는 크롭이 걸린 상태의 값이다.** 기본 상태는
  11.867 us 이고, 더 빠르므로 HD 60 FPS 상한 근거는 그대로 유지된다.

근거: pim-camera-v016 2대 교차 비교(드라이버 2.3 vs 현재, AP1302 펌웨어 동일 md5
`c0830fa0…`)와 현재 드라이버에서 `dz=125` 로 레지스터 단위 재현. 상세는 `CHANGELOG.md`
[Unreleased] 및 `docs/exposure-limits.md` §2.3.

## Version 2.12 (2026-09-04)

- 1280x720과 2560x720의 일반 FPS 협상 상한을 30에서 60으로 올렸다. AP1302 펌웨어의
  720p 라인타임이 14.80 us(60 FPS급)이고 보드 실측이 60 FPS 요청에서 54.0~55.5 FPS를
  기록했으므로, 기존 30은 하드웨어가 아니라 드라이버가 만든 상한이었다.
- 1920x1080과 3840x1080은 30을 유지한다. 1080p 라인타임 26.27 us로는 60 FPS 트리거
  주기 안에 프레임을 못 읽어 정수 트리거 분주로 떨어지고, 요청을 올릴수록 오히려
  나빠진다(40 → 19.9, 60 → 19.8 FPS).
- `EXP_TIME(0x500c)` 쓰기 안전 상한은 모든 모드에서 30 FPS 그대로다. 따라서 경고 후
  적용 구간은 640x360의 31~120 FPS와 1280x720의 31~60 FPS다. 1920x1080은 모드 상한도
  30이라 이 구간이 없다.
- 640x360 고속 preview 경로는 출력 해상도로 계속 게이트되며 HD가 상속하지 않는다.

**보드 검증(2026-09-04, pim-camera-v016).** 드라이버 2.12를 적재해 모드별 FPS 상한
11케이스를 확인했다 — 1280x720/2560x720은 60 ACCEPT / 61 REJECT, 1920x1080은
30 ACCEPT / 31 REJECT, 640x360은 120 ACCEPT / 121 REJECT다. 2.11에서 `-EINVAL`이던
HD 60 요청이 통과한다.

**HD 60의 실제 전달률은 측정하지 못했다.** `max_fps`는 드라이버가 허용하는 요청
상한이며, 그 FPS가 전달된다는 보장도 상위 스택이 그 조합을 지원한다는 보장도
아니다 — 640x360의 120이 요청 상한이지 전달 보장이 아닌 것과 같다.
