...
**⚠️ 이 릴리즈만으로는 운영에서 HD 60을 쓸 수 없다.** 드라이버 상한은 풀렸지만
gstApp에 같은 성격의 상한이 남아 있다. gstApp `max9296Prepare.cpp:22`의
`kMaxFpsHdFhd = 30`이 HD와 FHD를 한 상수로 묶고 있어, HD 60 요청이 prepare 이전에
`[MAX9296_PREPARE] invalid request`로 거부된다. HD 60을 실제로 쓰려면 gstApp 쪽
상한도 함께 올려야 한다.
...
HD 60 요청이 통과한다. **전달률은 위 gstApp 상한 때문에 측정하지 못했다.**
