...
### Changed
- prepare admission 게이트가 `power_count` 대신 실제 `streaming` 여부로만 거부한다.
  이 BSP 에서 `power_count` 는 살아있는 소유자의 증거가 아니다.
- 드라이버 버전 2.4 → 2.5
...
- 온타겟(pim-camera-v016): 누수 잔류 상태에서 prepare 성공, 그 구간 펌웨어
  ...
  스트리밍 중에는 여전히 `-EBUSY`
- 보드 게이트 G1~G3 통과, G4 dual 50/50 (`docs/prepare-board-gate-v1.md`)
