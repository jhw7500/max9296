...
- `max9296_360p_policy.h` 머리 주석도 같은 14.80 us 를 인용하고 있다. **주석 수정은 이
  문서 변경과 분리해 별도로 처리한다** — 동작 영향은 없다(컴파일되는 상수는
  `MAX9296_HD_MAX_FPS 60U` 뿐이고, 실측이 더 빨라 60 FPS 상한 근거는 유지된다).
