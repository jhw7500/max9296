...
- `preview_ae_max_et` (`...+0x102c`, 실제 ID `0x0098192c`) -> `0x2028`
  (u32 µs, 기본 33333, 노출 상한. 듀얼에서는 양 채널에 동일 적용. §3.3 참조)
...
crop 캐시 전체를 다시 적용한다. 한쪽 채널에만 값이 적용된 실패도 같은 방식으로
재시도하며, 이 오류만으로 자동 재시도나 보드 리셋을 실행하지 않는다.
...
`min(SENSOR_TOTAL_FRAME_TIME(AP1302 0x00fc), AP1302 0x2028)` 에서 클램프되고,
`0x2028`(PREVIEW_AE_MAX_ET)의 펌웨어 기본값이 33,333 us 다. 30 FPS 이하 모드에서는
프레임 주기가 아무리 길어도 노출이 33.3 ms 를 넘지 못한다는 뜻이다. 초과 요청은
거부되지 않고 **조용히 잘리며** `0x500c` 에는 요청값이 그대로 남으므로 로그나
`--get-ctrl` 만으로는 드러나지 않는다. 공용 `preview_ae_max_et` 컨트롤이 이 상한을
런타임에 조정한다 — 다른 런타임 컨트롤과 같이 세션 한정이며, 앱 재기동 시 JSON
기준으로 돌아간다.

하한도 있다. 센서 `COARSE_INTEGRATION_TIME` 의 최소값이 2행이라 그보다 짧은 노출은
요청해도 2행에 머문다. fps 와 무관하고 모드의 라인타임에만 비례한다.

실효 노출 확인은 AR0234 `R0x3012` 를 AP1302 DMA 로 읽는다
(`cam_ap1302_dma_verify.sh <ch> 0x3012`, 행 수 × 라인타임). 드라이버에는 readback 이
없어 이 경로가 유일한 실측 수단이다.

> 실측 (2026-09-18, dual-wide): 1080p@15fps 에서 상한을 60,000 으로 올리면 노출이
> 33,307 → 59,968 us 로 실제로 늘어난다(1268 → 2283 행). 라인타임은 1080p 26.267 /
> HD 11.867 / 360p 6.81 us, 하한 2행은 1080p 52.6 us · 360p 13.6 us 다. AE_CTRL 모드
> (`0x0290`/`0x0299`/`0x000c`) 와 무관하게 동일하게 걸린다 — AP1302 데이터시트는
> `0x2028` 을 auto-AE 전용 한계로 기술하지만 이 펌웨어는 manual 경로에도 적용한다.
> 상한을 프레임 주기 가까이(주기의 99%) 올리면 노출은 늘지 않고 fps 만 무너진다.
